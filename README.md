# CFTR Mutation Analysis 

**Isabella Tuble**

# Disease/Phenotype

**Cystic Fibrosis**

# Gene

**CFTR (Cystic Fibrosis Transmembrane Conductance Regulator)**

# Reference Transcript Accession

**NM_000492.4**  

# Reference Protein Accession

**NP_000483.3**  

# Documented Variant

**F508del (ΔF508, deletion of phenylalanine at codon 508)**

# ClinVar Accession

**VCV000007105.210 (F508del pathogenic variant)**

# Galaxy History Name

**Tuble_Cystic_fibrosis_CFTR_Mutation_Lab**  

# Date of Analysis

**September 16, 2026** 

## Documentation Notes

- Wild‑type CDS and protein sequences were recorded and translated.
  
- Documented mutation (F508del) was analyzed for CDS length, protein length, reading frame, and functional consequence.
  
- Artificial mutation (TCG deletion) was created and compared against wild‑type and F508del.
  
- Results were stored in Galaxy and exported to GitHub for reproducibility.
  
- Reference genome and annotation versions were documented in the README for transparency.
   

##  References

Hong JS, Tindall JM, Tindall SR, Sorscher EJ (2024) Mutation accumulation in H. sapiens F508del CFTR countermands dN/dS type genomic analysis. PLoS ONE 19(7): e0305832. https://doi.org/10.1371/journal.pone.0305832

Luo, S., Rollins, S., Schmitz-Abe, K., Tam, A., Li, Q., Shi, J., Lin, J., Wang, R., & Agrawal, P. B. (2024). The solute carrier family 26 member 9 modifies rapidly progressing cystic fibrosis associated with homozygous F508del CFTR mutation. Clinica chimica acta; international journal of clinical chemistry, 561, 119765. https://doi.org/10.1016/j.cca.2024.119765

Riepe, C., Wąchalska, M., Deol, K. K., Amaya, A. K., Porteus, M. H., Olzmann, J. A., & Kopito, R. R. (2024).  Small-molecule correctors divert CFTR-F508del from ERAD by stabilizing sequential folding states.  Molecular Biology of the Cell, 35(2), ar15.https://doi.org/10.1091/mbc.E23-08-0336
